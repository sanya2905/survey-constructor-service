...
async def update_survey(survey_id: uuid.UUID, payload: SurveyUpdate, db: AsyncSession = Depends(get_db)):
    res = await db.execute(select(Survey).where(Survey.id == survey_id))
    survey = res.scalar_one_or_none()
    if not survey:
        raise HTTPException(404, "Survey not found")
...
async def publish_survey(survey_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    res = await db.execute(select(Survey).where(Survey.id == survey_id))
    survey = res.scalar_one_or_none()
    if not survey:
        raise HTTPException(404, "Survey not found")
...
async def export_completed_responses(survey_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
